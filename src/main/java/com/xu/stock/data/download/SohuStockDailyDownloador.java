...
public class SohuStockDailyDownloador {
    ...
    public static List<String> buildUrls(Stock stock) {
        ...
        if (fromDate == null) {
            fromDate = DateUtil.stringToDate("2015-01-01");
        } else {
            fromDate = DateUtil.addDay(fromDate, 1);
        }

        if (DateDiffUtil.getWorkDay(fromDate, endDay) >= 0 && fromDate.before(endDay)) {
		...
		try {
            ...
            if (indexs != null) {
                for (int i = indexs.size() - 1; i >= 0; i--) {
                    ...
                    if (thisExrights.compareTo(BigDecimal.valueOf(1.05)) == 1 || thisExrights.compareTo(BigDecimal.valueOf(9.95)) == -1) {
